...
def IEEE(num):
    ...
    if (num < 0):
        signbit = [1]
        num = num * -1
    intpart = convert_to_binary_integer(
        math.floor(num))  #numbers before decimal
    intpart.pop(0)  #remove first 1
    floatpart = convert_to_binary_float(
        num - math.floor(num))  #numbers after decimal
    mantissa = intpart + floatpart
    exponent = len(intpart)
    exponent = exponent + 127  #bais
